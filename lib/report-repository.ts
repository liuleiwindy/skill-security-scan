...
function writeReportFile(report: ScanReport) {
  ensureReportsDir();
...
function readReportFile(id: string): ScanReport | null {
  ensureReportsDir();
...
export async function loadReport(id: string): Promise<ScanReport | null> {
  ...
  const report = USE_POSTGRES ? await readReportPostgres(id) : readReportFile(id);
  if (report) {

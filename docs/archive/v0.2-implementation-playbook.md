...
## V0.2.3: Prompt Injection Coverage (External-First)

## Task F: Prompt Injection Integration

Scope for v0.2.3:

1. `PI-1-INSTRUCTION-OVERRIDE`
2. `PI-2-PROMPT-SECRET-EXFIL`

Implementation order:

1. Run external prompt-injection path first (runtime direct Z.AI OpenAI-compatible `chat/completions`).
2. Use local deterministic PI rules only as fallback when external path is unavailable.
3. Do not add non-PI agent-risk families in this version.

Provider guidance:

1. Online provider target: Z.AI OpenAI-compatible endpoint.
2. Runtime uses direct API calls; Promptfoo is retained as eval harness for local/CI validation.
3. Required env: `ZAI_API_KEY`.
4. Optional env: `ZAI_API_BASE_URL`, `ZAI_PI_MODEL`.

Done when:

1. PI-1/PI-2 findings can be produced in report output.
2. External-first and fallback behavior are both test-covered.
3. Existing non-PI scan behavior is not regressed.
...
2. `tests/rules.test.ts`
   - prompt-injection local fallback rule matching
